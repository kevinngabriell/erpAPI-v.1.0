...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $update_purchase_order_query = "UPDATE purchaseOrder SET PODate = ?, POShipmentDate = ?, UpdateBy = ?, UpdateDt = ?, POStatus = ? WHERE PONumber = ?";
    $stmt_update_purchase_order = mysqli_prepare($connect, $update_purchase_order_query);
    mysqli_stmt_bind_param($stmt_update_purchase_order, 'ssssss', $formatted_purchase_order_date, $formatted_shipment_date, $insert_by, $currentDateTimeString, $purchase_order_status, $purchase_order_number);
