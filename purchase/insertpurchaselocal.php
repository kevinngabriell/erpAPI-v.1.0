...
if($_SERVER['REQUEST_METHOD'] === 'POST'){
    ...
    $purchase_order_ppn = $_POST['purchase_order_ppn'];
    ...
    // Extract date from the provided string
    $date_parts_ship = explode(' ', $purchase_order_shipment);
    $date_string_ship = $date_parts_ship[1] . ' ' . $date_parts_ship[2] . ' ' . $date_parts_ship[3];
    
    // Create DateTime object with the extracted date string
    $purchase_order_date_obj_ship = DateTime::createFromFormat('M d Y', $date_string_ship);
    $formatted_purchase_order_date_ship = $purchase_order_date_obj_ship->format("Y-m-d");

    $product_length = $_POST['product_length'];
    $insert_purchase_local_query = "INSERT INTO purchaseOrder (PONumber, PODate, POSupplier, POShipmentDate, POPayment, POOrigin, POStatus , POType, POCurrency, POPPN, InsertBy, InsertDt) VALUES ('$purchase_order_number','$formatted_purchase_order_date', '$purchase_order_supplier', '$formatted_purchase_order_date_ship', '$purchase_order_payment', '$purchase_order_origin', '$purchase_order_status', '$purchase_order_type', '$purchase_order_currency', '$purchase_order_ppn', '$insert_by', '$currentDateTimeString');";
