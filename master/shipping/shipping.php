...
function getAllShipping($conn, string $search = '', int $page = 1, int $limit = 10): void {
    $search = mysqli_real_escape_string($conn, $search);
    $page   = max(1, $page);
    $limit  = min(100, max(1, $limit));
    $offset = ($page - 1) * $limit;

    $where = $search !== '' ? "WHERE shipment_name LIKE '%$search%'" : '';

    $count_result = mysqli_query($conn, "SELECT COUNT(*) AS total FROM shipment $where");
    $total        = (int) mysqli_fetch_assoc($count_result)['total'];

    // Names contain month/period strings (e.g. "early january") — ordered chronologically, not alphabetically
    $order_by = "ORDER BY
        CASE
    ...
    $result = mysqli_query($conn, "SELECT shipment_id, shipment_name FROM shipment $where $order_by LIMIT $limit OFFSET $offset");
