...
// --- GET ALL ---
// Searchable by: PO_Status_Name
// ?params=search  &page=1  &limit=10
function getAllPurchaseStatus($conn, string $params = '', int $page = 1, int $limit = 10): void {
    $params = mysqli_real_escape_string($conn, $params);
    $page   = max(1, $page);
    $limit  = min(100, max(1, $limit));
    $offset = ($page - 1) * $limit;

    $where = $params !== '' ? "WHERE PO_Status_Name LIKE '%$params%'" : '';

    $countResult = mysqli_query($conn, "SELECT COUNT(*) AS total FROM purchaseStatus $where");
